...
class wpl_property
{
	...
	public static function finalize($property_id, $mode = 'edit', $user_id = NULL)
	{
        ...
        $update_query .= "`finalized`='1', `mls_id_num`=cast(`mls_id` AS unsigned), `last_finalize_date`='".date('Y-m-d H:i:s')."', ";
		if(wpl_global::check_access('confirm', $user_id)) $update_query .= "`confirmed`='1',";

		// Listing Expiration
        $listing_expiration_status = wpl_global::get_setting('lisexpr_status');
        if($listing_expiration_status and !wpl_global::check_addon('membership'))
        {
            $update_query .= "`expired`='0',";
        }
        elseif($listing_expiration_status)
        {
            $membership = new wpl_addon_membership();
            if(!$membership->is_expired($user_id)) $update_query .= "`expired`='0',";
        }
