...
abstract class wpl_property_listing_controller_abstract extends wpl_controller
{
	...
	public function display($instance = array())
    {
		...
		{
			// Import Message tpl
			if(wpl_users::is_administrator()) $this->message = sprintf(__("You don't have access to this menu! %s KB article might be helpful.", 'wpl'), '<a href="https://support.realtyna.com/index.php?/Default/Knowledgebase/Article/View/618/">'.__('this', 'wpl').'</a>');
			else
            {
                $this->message = __("You don't have access to see properties! Please login or register first.", 'wpl');
                $this->error_code = 401;
            }
