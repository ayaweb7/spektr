...
printf ("<div id='catalog' class='container px-4 my-4'>
    <h1 class='pb-2 border-bottom text-center test-custom'>%s</h1>", $myrow4['h1']);


// Выборка в цикле всех существующих категорий услуг и товаров
$result2 = mysqli_query($db, "SELECT * FROM goods ORDER BY category DESC");
...
$category='';
	...
	do
	{
		...
		printf ("<h2 class='lead_p text-center'>%s</h2>
			<div class='row g-4 py-3 row-cols-1 row-cols-lg-3'>", $myrow2['category']);
			...
			do
			{
				if ($myrow3['good'] != $good)
				{
					printf ("<div class='feature col'>
						...
						<div>
							<img src='img/objects/%s.jpg' class='d-block mx-lg-auto img-fluid' title='%s' width='264' height='191' loading='lazy'>
						</div>
...
$result4->close(); // Страницы - без сортировки и лимитов
