...
?>
    ...
    <div id="realties">
		<form name='form' action='shop_form_update.php' method='post'>
...
<?php 
$result = mysqli_query($db, "SELECT * FROM shops WHERE shop='$shop' ORDER BY name, characteristic");
$myrow = mysqli_fetch_array($result);
...
printf  ("          <tr class='absent' style='background-color:".($even?'white':'#eaeaea')."'>
                      <td><a href='shop_form_update.php?id=%s'>%s</a></td>
					  <td>%s</td>
					  <td>%s, %s</td>
					  <td>%s %s</td>
                      <td>%s %s</td>
