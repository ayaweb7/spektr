...
$result1 = mysqli_query($db, "SELECT * FROM settings WHERE page='mysql_shops'");
...
<?php 
$result = mysqli_query($db, "SELECT * FROM shops WHERE shop='$shop' ORDER BY name, characteristic, date");
$myrow = mysqli_fetch_array($result);
...
?>
					<tr><td colspan='4'></td><td style='background-color: #19910f; font: 1,2em bold Georgia, "Times New Roman", Times, serif; color: #FFF300;'><?php echo $sum.' руб.' ?></td></tr>

				<caption> <!-- Заголовок таблицы -->
                    <h1>Всего в магазине <em><?php echo $shop ?></em> оставлено: <em><?php echo $sum.' руб.' ?></em></h1>
                  </caption>
                </table> <!-- inventory -->
