...
$result1 = mysqli_query($db, "SELECT * FROM settings WHERE page='name_update'");
...
?>
    ...
    <div id="realties">
		<form name='form' action='name_form_update.php' method='post'>
...
printf  ("          <tr class='absent' style='background-color:".($even?'white':'#eaeaea')."'>
                      <td><a href='name_form_update.php?id=%s'>%s</a></td>
					  <td>%s</td>
					  <td>%s, %s</td>
					  <td>%s %s</td>
                      <td>%s %s</td>
