...
$result1 = mysqli_query($db, "SELECT * FROM settings WHERE page='mysql_prices'");
...
$result = mysqli_query($db, "SELECT * FROM shops WHERE price='$price'");
...
<?php 
$result = mysqli_query($db, "SELECT * FROM shops WHERE price='$price' ORDER BY name, characteristic, shop");
$myrow = mysqli_fetch_array($result);
...
?>
				...
				<caption> <!-- Заголовок таблицы -->
                    <h1>Товары с ценой <em><?php echo $price ?></em> рублей.</h1>
                  </caption>
                </table> <!-- inventory -->
