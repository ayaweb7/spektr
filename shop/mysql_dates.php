...
$result1 = mysqli_query($db, "SELECT * FROM settings WHERE page='mysql_dates'");
...
$result = mysqli_query($db, "SELECT * FROM shops WHERE date='$date'");
...
<?php 
$result = mysqli_query($db, "SELECT * FROM shops WHERE date='$date' ORDER BY name, characteristic, shop");
$myrow = mysqli_fetch_array($result);
...
?>
				...
				<caption> <!-- Заголовок таблицы -->
                    <h1>Товары, купленные <em><?php echo $date ?></em></h1>
                  </caption>
                </table> <!-- inventory -->
