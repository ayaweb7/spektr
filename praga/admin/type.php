...
$result = mysql_query("SELECT title,meta_d,meta_k,h1,h2,date,month FROM settings WHERE page='section'",$db);
...
?>
    ...
    <div id="realties">
        <h1><?php echo $myrow['h1'] ?></h1>
              <table id="inventory" width="99%" class="realty sortable">
...
<!-- Подключаем БЛОК выбора параметров -->
    <?php include ("blocks/parameters_section.php"); ?>
...
<?php include ("blocks/vybor_section.php");?>
...
printf  ("          <tr style='background-color:".($even?'white':'#eaeaea')."'>
                      ...
                      <td>%s 000</td>
                      <td>%s</td>
                    </tr>  ", $myrow['id'],$myrow['street'],$myrow['house'],$myrow['square'],$myrow['balcony'],$myrow['wall'],$myrow['floor'],$myrow['trade'],$myrow['price'],$myrow['firm']); 
$even=!$even;
}
